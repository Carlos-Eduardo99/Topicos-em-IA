...
#define disponibilidade_1 10 // quantidade disponível do ítem 1
#define disponibilidade_2 12 // quantidade disponível do ítem 2
#define disponibilidade_3 11 // quantidade disponível do ítem 3
#define disponibilidade_4 8 // quantidade disponível do ítem 4
#define disponibilidade_5 7 //quantidade disponível do ítem 5
...
int fitnessTotal; // armazena a soma de todos os fitness da população para usar na
seleção
...
void inicializaPopulacao(){
    ...
    populacao[cont][0]=inteiroAleatorio(disponibilidade_1);// determina quantidade tipo 1
    populacao[cont][1]=inteiroAleatorio(disponibilidade_2);// determina quantidade tipo 2
    populacao[cont][2]=inteiroAleatorio(disponibilidade_3);// determina quantidade tipo 3
    populacao[cont][3]=inteiroAleatorio(disponibilidade_4);// determina quantidade tipo 4
    populacao[cont][4]=inteiroAleatorio(disponibilidade_5);// determina quantidade tipo 5
...
void reproducao(){
    ...
    while(cont<num_ind){// repete até concluir a próxima geração
        ...
        while(pesoFilho1>limitePeso||pesoFilho2>limitePeso){ // repete até que os dois filhos sejam válidos limite de peso
            ...
            // determina aleatoriamente se o corte é entre os cromossomos 1 e 2 (<0,5) ou2 e 3 (>=0,5)
            if(aleatorio()<0.5){
                // [1]|       pai
                //    |[2][3] mae
                // [1] [2][3] filho 1
...
void mutacao(){
    ...
    for(cont=0; cont < num_ind; cont++){
        if((aleatorio()*100)<taxa_mutacao){ // verifica a possibilidade deste indivíduosofrer mutação
